...
- Every LLM call must have Pydantic schema validation on output
...
```
ai-powered-adaptive-interview-interview-system/
├── .github/
│   ├── copilot-instructions.md   ← you are here (auto-loaded)
│   ├── prompts/                  ← attach these in Copilot Chat
│   └── workflows/                ← GitHub Actions (created by 05-cicd prompt)
├── frontend/                     (Next.js 14)
│   ├── app/
│   ├── components/
│   ├── hooks/
...
2. Select **"Prompt..."**
3. Choose the relevant prompt from `.github/prompts/`

Or type directly in chat:
```
#file:.github/prompts/00-orchestrator.prompt.md
```

| Prompt File | Use When |
|---|---|
| `00-orchestrator.prompt.md` | Starting or resuming any phase — always start here |
| `01-scaffold.prompt.md` | First-time project setup with uv + Next.js |
| `02-backend.prompt.md` | Writing FastAPI routers, services, repositories |
| `03-ai-pipeline.prompt.md` | Implementing Groq/Gemini AI modules |
| `04-frontend.prompt.md` | Writing Next.js pages and components |
| `05-cicd.prompt.md` | GitHub Actions, Docker, linting config |
| `06-git.prompt.md` | Committing, branching, PR descriptions |
...
Never hardcode these values. Always read from environment. Never commit `.env`.
